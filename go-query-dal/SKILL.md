...
DAL 代码统一放在 `store` 目录/package，只编排数据库访问，不承载业务规则。一个 DAL 方法操作一个主要数据 model；查询方法统一使用 `SearchXxx`，返回数据表 model 经 `Deserialize()` 后的结果；每个 DB 方法必须有 timeout；查询条件来自已校验的 model 层 param；排序分页必须统一使用专门定义的 model 层 `AddFilter` 方法。
...
3. 定义接口：公开方法第一个参数是 `ctx context.Context`；字段和构造函数参数常见顺序为 DB/事务入口、依赖 DAO/repository、外部 client/helper、logger；DB、logger、依赖 DAO/client 等通过构造函数或明确字段注入，不能因为依赖为 nil 就跳过查询、写入或维护逻辑；DAL 接口保持 `Create/Search/Update/Delete` 等资源/动作级方法，查询统一使用通用 `SearchXxx`，不新增 `FindXxx`、`GetXxx`、`SearchXxxForUser`、`UpdateXxxForProject` 这类场景化方法；同一个 DAO struct 的实现方法统一使用指针接收者，receiver 统一命名为 `dal`。
4. 实现查询：带领域方法的 param 使用指针类型；开头按 `param = param.Serialize()`、`param.Check()` 顺序处理；查询字段规整依赖拥有字段的 param 公有 `Serialize()`；创建 `context.WithTimeout` 并使用 `WithContext(cancelCtx)`；跨表过滤优先使用子查询，不优先使用 `Join`。
5. 组织 GORM 链路：一个 DAL 方法里尽量从上到下完成完整查询链路，让使用者打开方法就能看懂查询方式；先初始化空结果切片，所有返回路径都不能返回 nil slice；表名来自主要数据 model 的 `TableName()`；先业务过滤，再 `Count`，再 `AddFilter`，最后 `Find` 和 `Deserialize()`。
...
- [ ] 只提供资源/动作级 `Create/Search/Update/Delete` 等接口；查询统一使用通用 `SearchXxx`，没有主动新增 `FindXxx`、`GetXxx` 或 `SearchXxxForUser`、`UpdateXxxForProject` 等场景化方法。
- [ ] 每个 DB 操作使用 timeout 和 `WithContext(cancelCtx)`。
- [ ] Param 定义在 model 同层；DAL 只消费已 `Serialize()` / `Check()` 的 param，不重复清洗或校验。
- [ ] `Search` 返回数据表 model 经 `Deserialize()` 后的结果；初始化空结果 slice，链路按过滤、`Count`、`AddFilter`、`Find`、`Deserialize()` 顺序完整呈现。
- [ ] 一个 DAL 方法只围绕一个主要数据 model 组织查询；跨表过滤优先使用子查询，没有不必要的 `Join`。
