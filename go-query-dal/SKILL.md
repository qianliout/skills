...
3. 定义接口：公开方法第一个参数是 `ctx context.Context`；字段和构造函数参数常见顺序为 DB/事务入口、依赖 DAO/repository、外部 client/helper、logger；DB、logger、依赖 DAO/client 等通过构造函数或明确字段注入；同一个 DAO struct 的实现方法统一使用指针接收者，receiver 统一命名为 `dal`；默认提供 `Create`、`Search`、`Update`、`Delete`。
...
- [ ] DAL/DAO 文件放在 `store` 目录/package 下。
...
- [ ] 同一个 DAO struct 的所有实现方法都使用指针接收者，receiver 统一命名为 `dal`，例如 `func (dal *XxxDao) SearchXxx(...)`，没有值接收者或其它 receiver 名。
...
- [ ] 所有常量统一放到项目定义的 `consts` 目录下，没有散落在 DAL、DAO 或函数体中。
- [ ] 新表或新功能的时间字段按毫秒级 `int64` 时间戳落库和查询；已有表/功能不因本规则强制迁移。
- [ ] 没有新增不必要的基础包装类型，如 `type Kind string`；DAL 查询参数和字段优先使用基础类型。
