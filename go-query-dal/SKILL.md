...
- [ ] Step 3: 实现查询
  ...
  - [ ] 先拼业务 where，再 `Count(&cnt)`，再调用 model 层的 `AddFilter(db, param.Filter)` 统一处理排序分页，最后 `Find(&res)`。
...
6. `Count(&cnt)`。
7. 调用 model 层 `AddFilter(db, param.Filter)` 统一处理排序和分页。
8. `Find(&res)`。
9. 对结果执行 `Deserialize()`。
10. 返回 `res, cnt, err`。
...
- [ ] `Search` 先 `Count`，再调用 model 层 `AddFilter` 处理排序分页，再 `Find`。
