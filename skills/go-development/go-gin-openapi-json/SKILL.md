...
- [ ] Step 0：必须先经 `$go` 路由进入本 Skill。
- [ ] Step 1：运行预检脚本，失败立即停止。
- [ ] Step 2：确认唯一目标接口。
- [ ] Step 3：读取规则与模板。
- [ ] Step 4：定位路由和 handler。
- [ ] Step 5：分析 HTTP 契约。
- [ ] Step 6：建立该 operation 专用 schema。
- [ ] Step 7：使用脚本确定输出路径。
- [ ] Step 8：组合全新的 OpenAPI JSON。
- [ ] Step 9：写入并校验。
- [ ] Step 10：报告结果。

### Step 0：经 `$go` 路由

必须先加载 `$go`，再加载本 Skill。禁止跳过 `$go` 直接生成或刷新 OpenAPI JSON。
