...
- 一个函数必须处于同一抽象层级。主流程必须包含 3 至 7 个有序业务步骤；步骤超过 7 个必须按真实子任务拆分，少于 3 个步骤必须保持内联。
...
- 新增函数或方法的返回值最多三个（包含 `error`）；超过三个时必须使用具名结果 struct。仅框架或接口强制签名、外部协议签名、生成适配器和既有公开兼容签名允许例外。
- 禁止为普通业务字段创建无必要的具名基础类型包装，例如 `type Kind string`、`type Status int64`。仅外部协议要求具名类型、该类型必须定义方法，或该类型构成明确且不可由 struct 表达的强类型边界时允许创建。
