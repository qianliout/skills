...
2. 加载 `references/api-layer-conventions.md`；同时遵循当前任务触发的 `references/code-style.md`、`references/model-hierarchy.md`、`references/service-layer.md`、`references/logging.md`。
...
6. handler 不访问 DB/GORM/DAL。
7. 使用项目 response helper 返回；列表响应包含 items、total、itemsPerPage、startIndex，空列表返回空切片。

## Reference Loading

生成、重构或评审 API/handler/controller 代码时，必须加载 `references/api-layer-conventions.md`。
