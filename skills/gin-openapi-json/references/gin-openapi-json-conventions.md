...
Use this reference when generating, refreshing, reviewing, or explaining OpenAPI JSON for a single selected Gin interface.
...
- Generate one final JSON document containing exactly one interface: one path with one HTTP method.
- If the user specifies an output path or filename, write there. Otherwise auto-generate a filename from the route (e.g. `PUT /api/v2/scanner/detect/policy` → `openapi_scanner_detect_policy.json`) and write to the project root.
- Infer `info.title` and `info.version` from project config, module/service name, or README.
...
- Require an explicit single target interface before generating docs.
- Accept only selectors that resolve to exactly one handler: method + path (e.g. `PUT /api/v2/scanner/detect/policy`) or handler function name (e.g. `UserAPI.CreateUser`).
- Reject ambiguous selectors (path prefix, route group, module/package) — ask the user to narrow to one specific interface.
- Scan the wider project only to resolve this single route's handler, models, response helpers, and dependencies.
- If no target interface is specified, stop and ask for the method + path or handler name.
...
- "用 gin-openapi-json 给 `PUT /api/v2/containerSec/scanner/security/detect/policy` 生成文档。"
- "为 `UserAPI.CreateUser` 生成可以导入 Apifox 的 OpenAPI JSON，输出到 `docs/user_create.json`。"
- "刷新 `DELETE /api/v1/users/:id` 的 OpenAPI 文档，只按最新代码重建。"

Expected result:

- Create exactly one new OpenAPI JSON file containing only the specified interface.
- Report the output path and validation result.
- Mention unresolved facts only when they could not be inferred from code.

## Creation Mode

- Every run creates a fresh new file from current code. Never merge with, diff against, or preserve data from any existing JSON.
- If the user specifies an output path, write there. Otherwise auto-generate a filename from the route's last 2-3 segments or handler name (prefixed with `openapi_`) and write to the project root.
- Create the parent directory if it does not exist.
- If the output would overwrite a non-OpenAPI file, ask before writing.

## Non-Goals

- Do not generate more than one operation per file.
- Do not maintain multiple document versions.
- Do not diff against or merge with old OpenAPI files.
- Do not preserve old fields, old endpoints, or old schemas for compatibility.
- Do not generate documentation for the whole project or route groups.
- Do not use runtime traffic, browser traces, or online environments.
...
1. Build a route inventory only as needed to match the requested selector: final path, HTTP method, handler symbol, and source file.
1. Confirm the selector resolves to exactly one route before proceeding.
1. Build a single operation record: summary, query/path/header parameters, request body schema name, response schema name, status code, tags, and unresolved notes.
1. Build a schema registry from every request, response, DTO, and model struct referenced by this single operation.
...
1. Run JSON parsing validation after writing.
1. Deliver the path and concise counts: 1 operation, schema count, unresolved items.
...
- Prefer project code-discovery tools when available. Fall back to `rg`, Go AST inspection, and direct file reads.
...
- If dynamic registration cannot be resolved, report the missing facts instead of guessing.
- If multiple router entry points exist, infer from the user request, package names, or bootstrapping code. Ask only when multiple plausible matches remain.
- Only the single specified route is in scope — do not add neighboring routes even if discovered.
...
- [ ] The final file is a full rebuild for the single selected interface from current code, not a merge with older docs.
- [ ] The single path method maps to the selected real Gin route.
...
- The user did not specify which interface to document.
...
Ask for the smallest useful missing input: method + path, handler name, or output path.
