...
- [ ] Step 2: 优化结构
  ...
  - [ ] 变量声明时尽量直接初始化；使用 `in := Hello{}`，不要写 `var in Hello` 后续再赋值。
- [ ] Step 3: 处理 Go 关键约束
  ...
  - [ ] 每个 goroutine 必须有 `recover` 防护，panic 要按项目约定记录日志或转成可观测错误。
...
- 不要用 `var in Hello` 这类空声明后再赋值的写法；能直接初始化时使用 `in := Hello{}`。
...
- [ ] 变量声明尽量直接初始化，没有不必要的 `var in Hello` 空声明。
...
- [ ] 每个 goroutine 都有 `recover` 防护，并按项目约定记录 panic。
- [ ] Go 文件已运行 `goimport`。
- [ ] 能运行测试时已运行相关 `go test`。
