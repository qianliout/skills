...
4. 优化职责归属：业务能力优先实现为对应 struct 的 pointer receiver 方法，尽量少写包内裸函数；同一个 struct 不能同时存在值接收者和指针接收者方法，发现混用时统一改成指针接收者；所有方法接收者都使用指针接收者，不能使用值接收者；避免无意义薄包装函数。
...
- [ ] 同一个 struct 没有混用值接收者和指针接收者；发现 `func (m Xxx) ...` 与 `func (m *Xxx) ...` 并存时，已统一改成 `func (m *Xxx) ...`。
