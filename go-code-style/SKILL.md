...
5. 收敛结构：常量按职责统一管理；非 model 公共 struct 按项目约定统一存放；请求/参数规整优先放到 `Normalize()` 这类 receiver 方法；这类会修改 receiver 并返回自身指针的方法，调用方必须用原变量接收返回值，不裸调也不另起变量承接规整结果。
6. 处理 Go 约束：入参/出参优先具体类型；slice/map 作为返回值时必须先实例化，所有返回路径都不能返回 nil slice/map；数值字段默认 `int64`；JSON tag 不直接使用 `omitempty`；I/O 传递上游 `ctx`。
...
- [ ] 请求/参数 struct 的 `Normalize()` 这类规整方法是 receiver 方法，直接修改原对象并返回自身指针；nil receiver 会初始化为空对象；调用方只要调用这类有返回值的方法，都必须用原变量接收返回值，例如 `req = req.Normalize()`、`input.Prompt = input.Prompt.Normalize()`、`opts = opts.FillDefault()`；调用侧没有 `req.Normalize()`、`opts.FillDefault()` 裸调，也没有 `normalized := req.Normalize()`、`prompt := input.Prompt.Normalize()` 这类另起变量承接规整结果；业务逻辑中没有散落的参数 trim/default 处理。
