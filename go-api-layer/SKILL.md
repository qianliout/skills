...
3. 定义结构：API struct 只持有 service、logger 或轻量依赖；字段和构造函数参数常见顺序为 service、轻量 helper/config、logger；依赖通过构造函数或明确字段注入，不能在 handler 内临时创建 service/DAL/client；handler 方法统一使用指针接收者，receiver 统一命名为 `api`。
...
- [ ] handler/API 方法都使用指针接收者，receiver 统一命名为 `api`，例如 `func (api *XxxAPI) Action(ctx *gin.Context)`，没有值接收者。
